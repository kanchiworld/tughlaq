...
map<string, string> g_mapArgs;
map<string, vector<string> > g_mapMultiArgs;
#ifdef QT_GUI
const char *walletUrl = "https://www.kanchiworld.com/";
const char *walletDownloadsUrl = "https://sites.google.com/a/kanchiworld.com/kanchi-wo/5-minutes-fairy-tales-pics/";
const char *forumsUrl = "http://www.kanchiworld.com";
bool fRestart = false;
bool fRescan = false;
bool fEncrypt = false;
...
#endif
//mbt: changed fBootstrapTurbo flag to true
//mbt: changed fBootstrapTurbo flag to false again 10.31.2020
...
{
    g_mapArgs.clear();
    g_mapMultiArgs.clear();

    g_mapArgs["-programpath"] = argv[0];
    g_mapMultiArgs["-programpath"].push_back(argv[0]);
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        g_mapArgs[psz] = pszValue;
        g_mapMultiArgs[psz].push_back(pszValue);
    }

    // New 0.6 features:
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, g_mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            ...
            if (g_mapArgs.count(singleDash) == 0)
                g_mapArgs[singleDash] = entry.second;
            name = singleDash;
        }

        // interpret -nofoo as -foo=0 (and -nofoo=0 as -foo=1) as long as -foo not set
        InterpretNegativeSetting(name, g_mapArgs);
    }
}

std::string GetArg(const std::string& strArg, const std::string& strDefault)
{
    if (g_mapArgs.count(strArg))
        return g_mapArgs[strArg];
    return strDefault;
}
...
{
    if (g_mapArgs.count(strArg))
        return atoi64(g_mapArgs[strArg]);
    return nDefault;
}
...
bool GetBoolArg(const std::string& strArg, bool fDefault)
{
    if (g_mapArgs.count(strArg))
    {
        if (g_mapArgs[strArg].empty())
            return true;
        return (atoi(g_mapArgs[strArg]) != 0);
    }
    return fDefault;
}
...
{
    g_mapArgs[strArg] = strValue;
    g_mapMultiArgs[strArg].push_back(strValue);
    return;
}
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Tughlaq
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Tughlaq
    // Mac: ~/Library/Application Support/Tughlaq
    // Unix: ~/.tughlaq
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tughlaq";
#else
    fs::path pathRet;
...
const boost::filesystem::path &GetProgramDir()
{
    ...
    if (g_mapArgs.count("-programpath")) {
        path = fs::system_complete(g_mapArgs["-programpath"]);
...
const boost::filesystem::path &GetDataDir(bool fNetSpecific)
{
    ...
    if (g_mapArgs.count("-datadir")) {
        path = fs::system_complete(g_mapArgs["-datadir"]);
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "tughlaq.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Tughlaq will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("Tughlaq"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
unsigned int luckynumber(const char *str)
{
	if ( !str )
	    return 0;
