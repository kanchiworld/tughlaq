...
{
    return GetBoolArg("-testnet", false) ? 32987 : 33987;
}
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop Tughlaq server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "Tughlaq server stopping";
}
...
{ //  name                      function                 safemd  unlocked
  //  ------------------------  -----------------------  ------  --------
    ...
    { "getblocktime",           &getblocktime,           true,   false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: Tughlaq-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
bool ClientAllowed(const boost::asio::ip::address& address)
{
    ...
    const vector<string>& vAllow = g_mapMultiArgs["-rpcallowip"];
...
{
public:
    AcceptedConnectionImpl( boost::asio::io_context& io_context, boost::asio::ssl::context &context, bool fUseSSL) :
...
void ThreadRPCServer(void* parg)
{
    ...
    strRPCUserColonPass = g_mapArgs["-rpcuser"] + ":" + g_mapArgs["-rpcpassword"];
    if (g_mapArgs["-rpcpassword"] == "")
    {
        unsigned char rand_pwd[32];
        RAND_bytes(rand_pwd, 32);
        string strWhatAmI = "To use tughlaqd";

        if (g_mapArgs.count("-server"))
            strWhatAmI = strprintf(_("To use the %s option"), "\"-server\"");
        else if (g_mapArgs.count("-daemon"))
    ...
    boost::asio::ssl::context context(ssl::context::sslv23);
    if (fUseSSL)
    {
    ...
    const bool loopback = !g_mapArgs.count("-rpcallowip");
...
void ThreadRPCServer3(void* parg)
{
    ...
    while (true)
    {
        ...
        if (!HTTPAuthorized(mapHeaders))
        {
            ...
            if (g_mapArgs["-rpcpassword"].size() < 20)
                MilliSleep(250);
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params) const
{
    ...
    // Observe safe mode
    string strWarning = Tughlaq::GetWarnings("rpc");
...
{
    if (g_mapArgs["-rpcuser"] == "" && g_mapArgs["-rpcpassword"] == "")
    ...
    string strUserPass64 = EncodeBase64(g_mapArgs["-rpcuser"] + ":" + g_mapArgs["-rpcpassword"]);
