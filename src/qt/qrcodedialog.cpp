...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TLQ unit input in lnReqAmount, we generate the URI with TLQ as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::formatMaxDecimals(BitcoinUnits::TLQ, ui->lnReqAmount->value(), BitcoinUnits::maxdecimals(BitcoinUnits::TLQ)));
            paramCount++;
        }
        else
        {
