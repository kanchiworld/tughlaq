...
void AskPassphrasePage::accept()
{
    ...
    case Encrypt: // Encrypt wallet and set password
        ...
        else
        {
            ...
            {
                ui->messageLabel->setText(tr("Tughlaq will now restart to finish the encryption process."));
		...
		std::cout << "no here either?" << std::endl;
		    std::cout << __FILE__ << __LINE__ << "page file frestart here = " << fRestart << std::endl;
                fRestart = true;
                MilliSleep(7 * 1000);
                StartShutdown();
            }
