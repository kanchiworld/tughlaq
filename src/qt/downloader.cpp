...
{
	std::cout << "Gautam: point 213 . frestart = " << fRestart << std::endl;
    downloaderQuit = true;
...
void Downloader::startRequest(QUrl url)
{
    ...
    if (processBlockchain)
    {
        statusText.append(tr("\n\nThe Tughlaq wallet will restart after extracting the bootstrap%1").arg((fEncrypt ? " and encrypting the wallet." : ".")));
    }
    else if (processUpdate)
    {
        statusText.append(tr("\n\nThe Tughlaq wallet will restart after downloading the update."));
    }
...
void Downloader::reloadBlockchain()
{
    ...
    {
	std::cout << "Gautam: point 203 . frestart = " << fRestart << std::endl;
        if (!walletModel->reloadBlockchain())
        {
	std::cout << "Gautam: point 204 . frestart = " << fRestart << std::endl;
            fBootstrapTurbo = false;
