...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(BitcoinUnits::TLQ)
    {
    ...
    processors(boost::thread::hardware_concurrency())
{
    ...
    printf("\nmbt: initial state of gen param [%d]", GetBoolArg("-gen") );
    printf("\nmbt: initial state of mining [%d]\n", mining);
    if (mining)
    {
    ...
    //mbt block the callback function invocation during the initial value set
    ui->spinBox->blockSignals(true);
    ui->spinBox->setRange(1,processors);
    ui->spinBox->setValue(processors-1);
...
void OverviewPage::setStatistics()
{
    ...
    ui->blocknumber->setText(QString::number(g_pindexBest->nHeight));
    ...
    ui->blockreward->setText(QString::number((double)Tughlaq::GetProofOfWorkReward(0,g_pindexBest->pprev)/COIN));
}
...
void OverviewPage::setModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("TLQ")
    updateDisplayUnit();
}
...
void OverviewPage::on_mineButton_clicked()
{
    ...
    // Gautam: Basic checks temporarily disabled. To be enabled again
//    if((secs > 90*60 && count < nTotalBlocks && !mining) || (peers < 1 && !mining))
//    {
	    printf("\nmbt: on_mineButton_clicked: count = %d, nTotalBlocks = %d, secs = %d, peers = %d\n", count, nTotalBlocks, secs, peers);
...
//    }
    ...
    if (!mining)
    {
        ...
        ui->mineButton->setIcon(QIcon(":/icons/miningon"));
        MilliSleep(100);
        GenerateTughlaq(true, g_tughlaqWallet);
    }
    else
    {
        GenerateTughlaq(false, g_tughlaqWallet);
        mining = false;
        ui->miningLabel->setText("Click to start:");
        ui->mineButton->setIcon(QIcon(":/icons/miningoff"));
    }
