...
void setupAmountWidget(QLineEdit *widget, QWidget *parent)
{
    ...
    amountValidator->setDecimals(BitcoinUnits::decimals(BitcoinUnits::TLQ));
...
{
    // Tughlaq: check prefix
    if(uri.scheme() != QString("tughlaq"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::TLQ, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert tughlaq:// to tughlaq:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("tughlaq://"))
    {
        uri.replace(0, 12, "tughlaq:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Tughlaq.lnk";
}
...
{
    return GetAutostartDir() / "tughlaq.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Tughlaq-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  tughlaq-qt [" + tr("command-line options") + "]                     " + "\n";
