    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s TLQ %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn)
{
    uint256 hash = wtxIn.GetHash();
    {
        ...
        if (fInsertedNew)
        {
            ...
            {
                if (g_mapBlockIndex.count(wtxIn.hashBlock))
                {
                    ...
                    unsigned int& blocktime = g_mapBlockIndex[wtxIn.hashBlock]->nTime;
...
bool CWallet::IsChange(const CTxOut& txout) const
{
    ...
    if (ScriptEngine::ExtractDestination(txout.scriptPubKey, address) && ScriptEngine::IsMine(*this, address))
    {
        LOCK(cs_wallet);
...
bool CWalletTx::AcceptWalletTransaction(CTxDB& txdb, bool fCheckInputs)
{

    {
        ...
        BOOST_FOREACH(CMerkleTx& tx, vtxPrev)
        {
            ...
            {
                uint256 hash = tx.GetHash();
                if (!g_mempool.exists(hash) && !txdb.ContainsTx(hash))
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s TLQ %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
        ...
        if (!vMissingTx.empty())
        {
            ...
            if (ScanForWalletTransactions(g_pindexGenesisBlock))
    ...
    BOOST_FOREACH(const CMerkleTx& tx, vtxPrev)
    {
        if (!tx.IsCoinBase())
        {
            ...
            if (!txdb.ContainsTx(hash))
                homeNode->RelayTransaction((CTransaction)tx, hash);
        }
    }
    if (!IsCoinBase())
    {
        ...
        if (!txdb.ContainsTx(hash))
        {
            ...
            homeNode->RelayTransaction((CTransaction)*this, hash);
        }
    }
}
...
void CWallet::ResendWalletTransactions()
{
    ...
    CTxDB txdb("r");
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (g_nTimeBestReceived - (int64_t)wtx.nTimeReceived > 5 * 60)
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                int nIn = 0;
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx*,unsigned int)& coin, setCoins)
                    if (!ScriptEngine::SignSignature(*this, *coin.first, wtxNew, nIn++))
                        return false;
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    NotifyAddressBookChanged(this, address, strName, ScriptEngine::IsMine(*this, address), (mi == mapAddressBook.end()) ? CT_NEW : CT_UPDATED);
    if (!fFileBacked)
        return false;
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    NotifyAddressBookChanged(this, address, "", ScriptEngine::IsMine(*this, address), CT_DELETED);
    if (!fFileBacked)
        return false;
        ...
        BOOST_FOREACH(PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (!IsMine(pcoin->vout[i]))
                    continue;
                if(!ScriptEngine::ExtractDestination(pcoin->vout[i].scriptPubKey, addr))
                    continue;
...
set< set<CTxDestination> > CWallet::GetAddressGroupings()
{
    ...
    BOOST_FOREACH(PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet)
    {
        ...
        if (pcoin->vin.size() > 0 && IsMine(pcoin->vin[0]))
        {
            ...
            {
                CTxDestination address;
                if(!ScriptEngine::ExtractDestination(mapWallet[txin.prevout.hash].vout[txin.prevout.n].scriptPubKey, address))
                    continue;
            ...
            BOOST_FOREACH(CTxOut txout, pcoin->vout)
                if (IsChange(txout))
                {
                    ...
                    if(!ScriptEngine::ExtractDestination(txout.scriptPubKey, txoutAddr))
                        continue;
        ...
        for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            ...
            {
                CTxDestination address;
                if(!ScriptEngine::ExtractDestination(pcoin->vout[i].scriptPubKey, address))
                    continue;
                grouping.insert(address);
                groupings.insert(grouping);
                grouping.clear();
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s TLQ %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s TLQ %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
void CWallet::GetKeyBirthTimes(std::map<CKeyID, int64_t> &mapKeyBirth) const {
    ...
    CBlockIndex *pindexMax = Tughlaq::FindBlockByHeight(std::max(0, g_nBestHeight - 144)); // the tip can be reorganised; use a 144-block safety margin
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); it++)
    {
        ...
        std::map<uint256, CBlockIndex*>::const_iterator blit = g_mapBlockIndex.find(wtx.hashBlock);

        if (blit != g_mapBlockIndex.end() && blit->second->IsInMainChain())
       	{
            ...
            BOOST_FOREACH(const CTxOut &txout, wtx.vout)
	    {
                ...
                ScriptEngine::ExtractAffectedKeys(*this, txout.scriptPubKey, vAffected);
