...
void CDB::Close()
{
    ...
    if (IsChainFile(strFile))
        nMinutes = 2;
    if (IsChainFile(strFile) && Tughlaq::IsInitialBlockDownload())
        nMinutes = 5;
...
bool CAddrDB::Write(const CAddrMan& addr)
{
    ...
    ssPeers << FLATDATA(g_pchMessageStart);
    ssPeers << addr;
...
bool CAddrDB::Read(CAddrMan& addr)
{
    ...
    try {
        // de-serialize file header (g_pchMessageStart magic number) and
        ssPeers >> FLATDATA(pchMsgTmp);

        // verify the network matches ours
        if (memcmp(pchMsgTmp, g_pchMessageStart, sizeof(pchMsgTmp)))
