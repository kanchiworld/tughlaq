...
public:
    ...
    {
        bn = BN_new();
	//mbt: do some good error checks here
    }
    ...
    */
    ...
    std::string ToString(int nBase=10) const
    {
        ...
        std::string str;
        CBigNum cbn = *this;
        BN_set_negative(&cbn, false);
        CBigNum dv;
        CBigNum rem;
        if (BN_cmp(&cbn, &bn0) == 0)
            return "0";
        while (BN_cmp(&cbn, &bn0) > 0)
        {
            if (!BN_div(&dv, &rem, &cbn, &bnBase, pctx))
                throw bignum_error("CBigNum::ToString() : BN_div failed");
            cbn = dv;
            unsigned int c = rem.getulong();
            str += "0123456789abcdef"[c];
        }
        if (BN_is_negative(&cbn))
            str += "-";
