...
{
    CNode::ClearBanned();
    g_mapArgs["-banscore"] = "111"; // because 11 is my favorite number
    CAddress addr1(ip(0xa0b0c001));
    CNode dummyNode1(INVALID_SOCKET, addr1, "", true);
...
BOOST_AUTO_TEST_CASE(DoS_mapOrphans)
{
    ...
    for (int i = 0; i < 50; i++)
    {
        ...
        tx.vout[0].nValue = 1*CENT;
        tx.vout[0].scriptPubKey.SetDestination(key.GetPubKey().GetID());
	ScriptEngine::SignSignature(keystore, txPrev, tx, 0);
...
BOOST_AUTO_TEST_CASE(DoS_checkSig)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++)
        BOOST_CHECK(ScriptEngine::SignSignature(keystore, orphans[j], tx, j));
    ...
    for (unsigned int i = 0; i < 5; i++)
        for (unsigned int j = 0; j < tx.vin.size(); j++)
            BOOST_CHECK(ScriptEngine::VerifySignature(orphans[j], tx, j, true, SIGHASH_ALL));
    ...
    BOOST_CHECK(!ScriptEngine::VerifySignature(orphans[0], tx, 0, true, SIGHASH_ALL));
    tx.vin[0].scriptSig = save;

    // Swap signatures, validation should fail:
    std::swap(tx.vin[0].scriptSig, tx.vin[1].scriptSig);
    BOOST_CHECK(!ScriptEngine::VerifySignature(orphans[0], tx, 0, true, SIGHASH_ALL));
    BOOST_CHECK(!ScriptEngine::VerifySignature(orphans[1], tx, 1, true, SIGHASH_ALL));
    std::swap(tx.vin[0].scriptSig, tx.vin[1].scriptSig);

    // Exercise -maxsigcachesize code:
    g_mapArgs["-maxsigcachesize"] = "10";
    // Generate a new, different signature for vin[0] to trigger cache clear:
    CScript oldSig = tx.vin[0].scriptSig;
    BOOST_CHECK(ScriptEngine::SignSignature(keystore, orphans[0], tx, 0));
    BOOST_CHECK(tx.vin[0].scriptSig != oldSig);
    for (unsigned int j = 0; j < tx.vin.size(); j++)
        BOOST_CHECK(ScriptEngine::VerifySignature(orphans[j], tx, j, true, SIGHASH_ALL));
    g_mapArgs.erase("-maxsigcachesize");
