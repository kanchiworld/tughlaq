...
Verify(const CScript& scriptSig, const CScript& scriptPubKey, bool fStrict)
{
    ...
    return ScriptEngine::VerifyScript(scriptSig, scriptPubKey, txTo, 0, fStrict, 0);
}
...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
        {
            ...
            bool sigOK = ScriptEngine::VerifySignature(txFrom, txTo[i], 0, true, 0);
            if (i == j)
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    txTo.vin[0].prevout.n = 0;
    txTo.vin[0].prevout.hash = txFrom.GetHash();
    BOOST_CHECK(ScriptEngine::SignSignature(keystore, txFrom, txTo, 0));
    txTo.vin[1].prevout.n = 1;
    txTo.vin[1].prevout.hash = txFrom.GetHash();
    BOOST_CHECK(ScriptEngine::SignSignature(keystore, txFrom, txTo, 1));
    txTo.vin[2].prevout.n = 2;
    txTo.vin[2].prevout.hash = txFrom.GetHash();
    BOOST_CHECK(ScriptEngine::SignSignature(keystore, txFrom, txTo, 2));
