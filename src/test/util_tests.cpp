...
BOOST_AUTO_TEST_CASE(util_ParseParameters)
{
    ...
    BOOST_CHECK(g_mapArgs.empty() && g_mapMultiArgs.empty());
    ...
    BOOST_CHECK(g_mapArgs.size() == 3 && g_mapMultiArgs.size() == 3);
    BOOST_CHECK(g_mapArgs.count("-a") && g_mapArgs.count("-b") && g_mapArgs.count("-ccc") 
                && !g_mapArgs.count("f") && !g_mapArgs.count("-d"));
    BOOST_CHECK(g_mapMultiArgs.count("-a") && mapMultiArgs.count("-b") && mapMultiArgs.count("-ccc") 
                && !g_mapMultiArgs.count("f") && !mapMultiArgs.count("-d"));

    BOOST_CHECK(g_mapArgs["-a"] == "" && g_mapArgs["-ccc"] == "multiple");
    BOOST_CHECK(g_mapMultiArgs["-ccc"].size() == 2);
}

BOOST_AUTO_TEST_CASE(util_GetArg)
{
    g_mapArgs.clear();
    g_mapArgs["strtest1"] = "string...";
    // strtest2 undefined on purpose
    g_mapArgs["inttest1"] = "12345";
    g_mapArgs["inttest2"] = "81985529216486895";
    // inttest3 undefined on purpose
    g_mapArgs["booltest1"] = "";
    // booltest2 undefined on purpose
    g_mapArgs["booltest3"] = "0";
    g_mapArgs["booltest4"] = "1";
