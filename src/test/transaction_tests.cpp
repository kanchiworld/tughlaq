    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if (test[0].type() == array_type)
        {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++)
            {
                ...
                BOOST_CHECK_MESSAGE(ScriptEngine::VerifyScript(tx.vin[i].scriptSig, mapprevOutScriptPubKeys[tx.vin[i].prevout], tx, i, test[2].get_bool(), 0), strTest);
            }
        }
    }
}
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if (test[0].type() == array_type)
        {
            ...
            for (unsigned int i = 0; i < tx.vin.size() && fValid; i++)
            {
                ...
                fValid = ScriptEngine::VerifyScript(tx.vin[i].scriptSig, mapprevOutScriptPubKeys[tx.vin[i].prevout], tx, i, test[2].get_bool(), 0);
            }
